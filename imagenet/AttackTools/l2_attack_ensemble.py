...
class CarliniL2Ensemble:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        output_nl, _ = model.predict(input_nhwc)
        self.pred_label = tf.argmax(output_nl, 1)
        self.output = output_nl
    ...
    def attack_batch(self, imgs, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                # perform the attack 
                _, l, l1, pls, output, l2s, scores_nvl, nimg = self.sess.run([self.train, self.loss, self.loss1, self.pred_label, self.output,
                                                         self.l2dist, self.output_nvl, 
                                                         self.newimg])
                # print pl, l, l1, l2s, scores_nvl.shape, nimg.shape
                # print nimg[0,:10,0,0]
                # print output
                # _, l, l1, pl, output, l2s, scores_nvl, nimg = self.sess.run([self.train, self.loss, self.loss1, self.pred_label, self.output,
                #                                          self.l2dist, self.output_nvl,
                #                                          self.newimg])
                # print pl, l, l1, l2s, scores_nvl.shape, nimg.shape
                # print nimg[0,:10,0,0]
                ...
                if iteration%(self.MAX_ITERATIONS//10) == 0: # %%%
                    ...
                    print(iteration, l, l1, l2s, pls[:1])
                    # print(pl)
                ...
                for e,(l2,sc_vl,ii,pl) in enumerate(zip(l2s,scores_nvl,nimg,pls[:1])):
        ...
        print 'best l2:',o_bestl2
        return o_bestattack, o_bestl2[0], o_bestpl[0]
